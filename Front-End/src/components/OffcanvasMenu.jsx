...
import GerenciarCategorias from './GerenciarCategorias'; // Importação do novo modal/tela cheia

function OffcanvasMenu({ showProfile, setShowProfile, usuarioLogado, handleLogout, temaAtual, toggleTema }) {
  const isDark = temaAtual === 'dark';
  const [showGerenciar, setShowGerenciar] = useState(false); // Controle da tela de Categorias
  ...
  return (
    <>
        ...
        <Offcanvas.Body className="p-0 d-flex flex-column">
          ...
          <div className="px-3 pt-4 flex-grow-1 overflow-auto">
            ...
            <div className={`card ${isDark ? 'dark-card bg-dark' : 'bg-white'} border-0 mb-4 shadow-sm`} style={{ borderRadius: '1rem' }}>
              ...
              {/* OPÇÃO DE GERENCIAR CATEGORIAS ATUALIZADA */}
              <div 
                className={`d-flex align-items-center justify-content-between p-3 border-bottom ${isDark ? 'border-secondary border-opacity-25' : 'border-light'}`} 
                style={{ cursor: 'pointer' }}
                onClick={() => {
                  setShowGerenciar(true); // Abre a tela cheia
                  setShowProfile(false);  // Fecha a gaveta lateral
                }}
              >
                <div className="d-flex align-items-center gap-3">
                  {/* Ícone agora padronizado com o estilo minimalista (sem azul) */}
      ...
      {/* Renderiza a tela de categorias (agora é full-screen) */}
      <GerenciarCategorias 
        show={showGerenciar} 
        onHide={() => setShowGerenciar(false)} 
